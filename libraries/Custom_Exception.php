...
class Custom_Exception
{
    ...
    public function handle_exception($file, $line, $message, $callback = null)
    {
        ...
        $log['get'] = $this->ci->input->get();
        $log['post'] = $this->ci->input->post();
        $log['files'] = $_FILES;
        $log['is_ajax'] = $this->ci->input->is_ajax_request();
        $log['is_cli'] = is_cli();
        ...
        if (isset($this->ci->session)) {
            $log['sessiondata'] = json_encode($this->ci->session->userdata());
        } else {
            $log['sessiondata'] = 'Session Library not loaded.';
        }

        /*
         * Retrieve Stacktrace
         */
        $log['stacktrace'] = $this->_get_stacktrace();

        /*
         * If Database is loaded get last query and last error
         */
        if (isset($this->ci->db) && isset($this->ci->db->conn_id) && $this->ci->db->conn_id != '') {
            $log['sqlquery'] = $this->ci->db->last_query();
            $log['sqlerror'] = json_encode($this->ci->db->error());
            ...
            $log['sqlquery'] = 'Database Library not loaded.';
            $log['sqlerror'] = 'Database Library not loaded.';
        }
        ...
        if (ENVIRONMENT == 'production') {
            if ($this->_email_send) {
                ...
                $msg .= '<p>File: ' . $log['file'] . ' line ' . $log['line'] . '</p>';
                $msg .= '<p>Error: ' . $log['message'] . '</p>';
                $msg .= '<p>Session Data: </p>';
                $msg .= $log['sessiondata'];
                $msg .= '<p>Last SQL Query: ' . $log['sqlquery'] . '</p>';
                $msg .= '<p>Last SQL Error: ' . $log['sqlerror'] . '</p>';
                $msg .= '<p>Stacktrace: </p>';
                $msg .= $log['stacktrace'];
            ...
            if ($callback !== null) {
                ...
                redirect($this->_default_callback . '/' . rawurlencode($message));
            } else {
                redirect(base_url());
            }
        } else {
